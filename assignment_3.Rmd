...
Vérifier qu'il faut pas recréer les jeu de données à chaque fois pour spécifier le nombre d'observations de chaque groupe dedans.
...
stress_shift_replications <- stress_shift_replications %>%
  dplyr::mutate(
    Permutation_Pvalue=v_pdp_pvalue_right(Noun_N_Syll_2, Verb_N_Syll_1,
                                          651,
                                          673,
                                          n_samples=99))
```

+ with the same overall number of observations, but with one tenth as many observations for verbs as for nouns (12034
noun and 1204 verb observations)
```{r second_condition, cache=T}
Noun_N_Syll_2 <- rbinom(1000, prob=proportion_n_syll_1, size=12034)
Verb_N_Syll_2 <- rbinom(1000, prob=proportion_v_syll_1, size=1204)
stress_shift_replications_3 <- stress_shift_replications %>%
  dplyr::mutate(Permutation_Pvalue=v_pdp_pvalue_right(Noun_N_Syll_1, Verb_N_Syll_1,12034,1204,n_samples=99))
```
...
stress_shift_replications_4 <- stress_shift_replications %>%
  dplyr::mutate(Permutation_Pvalue=v_pdp_pvalue_right(Noun_N_Syll_1, Verb_N_Syll_1,16,17,n_samples=99))
```


+ with a total of 33 observations, but with one tenth as many observations for verbs as for nouns (30 noun observations
and 3 verb observations)
```{r, eval=FALSE, evaluate=F, include=FALSE}
stress_shift_replications_5 <- stress_shift_replications %>%
  dplyr::mutate(Permutation_Pvalue=v_pdp_pvalue_right(Noun_N_Syll_1, Verb_N_Syll_1,30,3,n_samples=99))
```
...
Noun_N_Syll_1_2 <- rbinom(1000, prob=0.52, size=nb_n_observations)
Verb_N_Syll_1_2 <- rbinom(1000, prob=0.48, size=nb_v_observations)


# on utilise ces données pour créer notre table
stress_shift_replications_2 <- data.frame(
  "Noun_Percent_Syll_1" = Noun_N_Syll_1_2,
  "Verb_Percent_Syll_1" = Verb_N_Syll_1_2,
  "Replication" = paste0("R", sprintf("%04d", 1:1000)),
  "Difference_in_Proportion" = Noun_N_Syll_1_2/6506 - Verb_N_Syll_1_2/6732
)

stress_shift_replications_6 <- stress_shift_replications_2 %>%
  dplyr::mutate(Permutation_Pvalue=v_pdp_pvalue_right(Noun_N_Syll_1_2, Verb_N_Syll_1_2,651,673,n_samples=99))
```
...
```{r, eval=FALSE, evaluate=F, include=FALSE}
# on peut réutiliser les données crées dans l'étape précédente car les proba sont les mêmes

stress_shift_replications_6 <- stress_shift_replications_2 %>%
  dplyr::mutate(Permutation_Pvalue=v_pdp_pvalue_right(Noun_N_Syll_1_2, Verb_N_Syll_1_2,6506,6732,n_samples=99))
```
...
## for each of the values that "Syllable" can take, compare the proportions for a particular group
